...
{
    m_lastIndex = -1;
    m_max = m_min = m_max_pos = m_min_pos = 0;
}
...
// Mou tots els valors des de la posició idx a idx+1
void mcj_enters::move(unsigned int idx)
{
    for (unsigned int i = card(); i >= idx; --i)
    {
        m_cj[i] = m_cj[i - 1];
    }
}
